...
## En este repo (draw_automatic)

Indexado: 149 ficheros `.py`, `.js` y `.mjs` (2.598 nodos, 6.144 aristas). El build
completo tarda ~6 s y es local.

**Lo que graft NO indexa aquí**, y donde por tanto sí toca `grep` / lectura directa:

- `templates/*.html` — las plantillas Jinja, incluidas las config inyectadas como
  `<script type="application/json">` (`drawio-page-config`, `diagram-glpi-config`,
  `upload-glpi-config`).
- `static/style.css` y el resto de CSS.
- `library/libreria_Ausarta_JUN_2026.xml` — la librería de formas draw.io.

El JS de `static/js/` **sí** está indexado, así que para el frontend de comportamiento
(los IIFE de `creation-form*.js`, `upload-glpi-form.js`…) usa graft; solo el HTML y el CSS
quedan fuera.

**Nunca `graft build --deep`.** Manda código de clientes a un LLM externo, y este repo
lleva datos de clientes reales de AUSARTA. El build normal (sin `--deep`) es 100 % local y
es el único que hace falta.

`graft/` está en `.gitignore`: es caché local. Quien clone el repo ejecuta `graft build`.
